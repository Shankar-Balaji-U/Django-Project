...
(function($) {
	$.fn.formset = function(opts) {
			...
			if (addButton === null) {
				if ($this.prop("tagName") === "TR") {
					...
					if(options.firstValuePrefix && options.secondValuePrefix && options.subTotalPrefix){
						addSubTotal(0);             // 0 -> is the index value
						addGrandTotal(1);           // 1 -> is the No. of the row
		...
		const addSubTotal = function(index) {
			// var rowLen = $(".dynamic-form").length;
		...
		const addGrandTotal = function(length) {
			...
			for(var index1=0; index1 < length; index1++) {
				var firstTotalId = `#id_${options.prefix}-${index1}-${options.firstValuePrefix}`;
				var secondTotalId = `#id_${options.prefix}-${index1}-${options.secondValuePrefix}`;
				$(firstTotalId + "," + secondTotalId).on("input change", function(){
					var gtotal=0;
					for(var index2=0; index2 < length; index2++) {
						gtotal = gtotal + Number($(`#id_${options.prefix}-${index2}-${options.subTotalPrefix}`).val());
					}
					$(grandTotalId).val(gtotal);
				});
	...
	$.fn.tabularFormset = function(selector, options) {
		...
		const rowDeleteWithAjax = function(id) {
			$.ajax({
				type: "POST",
				url: "/invoiceitem/delete/" + id + "/",
				data: {
					'pk': id,
		...
		const addDeleteRowInput = function(row) {
			const regex = new RegExp("(id_" + options.prefix + "-\\d+-id)");
			let content = row.find(".original input[type=hidden]:last");
			let row_id = content.val();  //.match(regex)

			let containerId = "#delitem-dialoguebox",
				confirmButton = "#deletebutton",
				cancelButton = "#cancelbutton",
				itemName = "#item-name";

			if(options.isItemNew == false) {
	            $(cancelButton).on("click", function() {
	                // $(containerId).removeClass('show-delete-container').addClass('hide-delete-container');
	            });
	            $(confirmButton).on("click", function() {
	                rowDeleteWithAjax(row_id);
	            });
			}
		};
