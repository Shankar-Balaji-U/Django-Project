...
(function (global, factory) {
  ...
  */
  ...
  /**
   * ------------------------------------------------------------------------
   * Import element, bootstrapDelegationHandler functions
   * ------------------------------------------------------------------------
   */
  ...
  /**
   * ------------------------------------------------------------------------
   * Add Manipulator Methods
   * ------------------------------------------------------------------------
   */
  Manipulator.style = (element, style) => { Object.assign(element.style, style) };

  Manipulator.toggleClass = function(element, className) { 
    if (!element) {
      return;
    }
  ...
  Manipulator.addClass = function(element, className) {
    if (element.classList.contains(className)) return;
    element.classList.add(className);
  }

  Manipulator.addStyle = function(element, style) {
    Object.keys(style).forEach((property) => {
      element.style[property] = style[property];
    });
  }

  Manipulator.removeClass = function(element, className) {
    if (!element.classList.contains(className)) return;
    element.classList.remove(className);
  }

  Manipulator.hasClass = (element, className) => { return element.classList.contains(className) };


  /**
   * ------------------------------------------------------------------------
   * Add SelectorEngine Methods
   * ------------------------------------------------------------------------
   */

  SelectorEngine.closest = (element, selector) => { return element.closest(selector) };
  SelectorEngine.matches = (element, selector) => { return element.matches(selector) };
  ...
  const CLASSNAME_VIEW_PASSWORD$i = 'password-viewer';
  ...
  const EVENT_MOUSEUP_DISMISS$i = `mouseup.dismiss${EVENT_KEY$i}`;
  const EVENT_MOUSEDOWN_DISMISS$i = `mousedown.dismiss${EVENT_KEY$i}`;
  /**
   * ------------------------------------------------------------------------
   * Class Definition
   * ------------------------------------------------------------------------
  ...
  class Input {
    constructor(element) {
      ...
      if (this._element) {
        Data.set(element, DATA_KEY$i, this);
        this.init();
      }
    }
    ...
    forceActive() {
      Manipulator.addClass(this.input, CLASSNAME_ACTIVE$i);
    }

    forceInactive() {
      Manipulator.removeClass(this.input, CLASSNAME_ACTIVE$i);
    }

    dispose() {
      this._removeBorder();

      Data.remove(this._element, DATA_KEY$i);
    ...
    _getHelper() {
      this._helper = SelectorEngine.findOne(SELECTOR_HELPER$i, this._element);
    }
    ...
    _showCounter() {
      ...
      this._counterElement = createElement('div');
      Manipulator.addClass(this._counterElement, CLASSNAME_COUNTER$i);
    ...
    _showPlaceholder() {
      Manipulator.addClass(this.input, CLASSNAME_PLACEHOLDER_ACTIVE$i);
    }

    _getPassword(input = this.input) {
      const password = Manipulator.getDataAttribute(input, 'password');
      const isTypePassword = input.getAttribute('type') === 'password';
      input.value = password;
      if(password === "toggle" && isTypePassword) {
        this._showPassword();
      }
    }
    _showPassword() {
      const viewer = createElement("span");
      Manipulator.addClass(viewer, CLASSNAME_VIEW_PASSWORD$i);
      Manipulator.addClass(viewer, "eye-close");
      this.input.after(viewer);

      EventHandler.on(viewer, EVENT_MOUSEUP_DISMISS$i, () => {
        this.input.type = "password";
        Manipulator.addClass(viewer, "eye-close");
      });

      EventHandler.on(viewer, EVENT_MOUSEDOWN_DISMISS$i, () => {
        this.input.type = "text";
        Manipulator.addClass(viewer, "eye-open");
      });
    }

    _getNotchData() {
      this._notchMiddle = SelectorEngine.findOne(SELECTOR_NOTCH_MIDDLE$i, this._element);
      this._notchLeading = SelectorEngine.findOne(SELECTOR_NOTCH_LEADING$i, this._element);
    }
    ...
    _applyDivs() {
      const allNotchWrappers = SelectorEngine.find(SELECTOR_NOTCH$i, this._element);
      const notchWrapper = createElement('div');
      Manipulator.addClass(notchWrapper, CLASSNAME_NOTCH$i);
      this._notchLeading = createElement('div');
      Manipulator.addClass(this._notchLeading, CLASSNAME_NOTCH_LEADING$i);
      this._notchMiddle = createElement('div');
      Manipulator.addClass(this._notchMiddle, CLASSNAME_NOTCH_MIDDLE$i);
      this._notchTrailing = createElement('div');
      Manipulator.addClass(this._notchTrailing, CLASSNAME_NOTCH_TRAILING$i);
    ...
    _removeBorder() {
      const border = SelectorEngine.findOne(SELECTOR_NOTCH$i, this._element);
    ...
    _activate(event) {
      onDOMContentLoaded(() => {
        ...
        if (input.value !== '') {
          Manipulator.addClass(input, CLASSNAME_ACTIVE$i);
        }
    ...
    static jQueryInterface(config, options) {
      return this.each(function () {
        let data = Data.get(this, DATA_KEY$i);
    ...
    static getInstance(element) {
      return Data.get(element, DATA_KEY$i);
    }
  ...
  EventHandler.on(document, 'focus', SELECTOR_OUTLINE_INPUT$i, Input.activate(new Input()));
  EventHandler.on(document, 'input', SELECTOR_OUTLINE_INPUT$i, Input.activate(new Input()));
  EventHandler.on(document, 'blur', SELECTOR_OUTLINE_INPUT$i, Input.deactivate(new Input()));

  EventHandler.on(document, 'focus', SELECTOR_OUTLINE_TEXTAREA$i, Input.activate(new Input()));
  EventHandler.on(document, 'input', SELECTOR_OUTLINE_TEXTAREA$i, Input.activate(new Input()));
  EventHandler.on(document, 'blur', SELECTOR_OUTLINE_TEXTAREA$i, Input.deactivate(new Input()));

  EventHandler.on(window, 'shown.bs.modal', (e) => {
    SelectorEngine.find(SELECTOR_OUTLINE_INPUT$i, e.target).forEach((element) => {
    ...
    SelectorEngine.find(SELECTOR_OUTLINE_TEXTAREA$i, e.target).forEach((element) => {
      const instance = Input.getInstance(element.parentNode);
      if (!instance) {
        return;
      }
  ...
  EventHandler.on(window, 'shown.bs.dropdown', (e) => {
    ...
    if (target) {
      SelectorEngine.find(SELECTOR_OUTLINE_INPUT$i, target).forEach((element) => {
        const instance = Input.getInstance(element.parentNode);
        if (!instance) {
          return;
        }
      ...
      SelectorEngine.find(SELECTOR_OUTLINE_TEXTAREA$i, target).forEach((element) => {
        const instance = Input.getInstance(element.parentNode);
        if (!instance) {
          return;
        }
  ...
  // auto-init
  SelectorEngine.find(`.${CLASSNAME_WRAPPER$i}`).map((element) => new Input(element));
  ...
  const NAME$r = 'ripple';
  const DATA_KEY$r = 'bs.ripple';
  const CLASSNAME_RIPPLE = 'ripple-surface';
  const CLASSNAME_RIPPLE_WAVE = 'ripple-wave';
  const SELECTOR_COMPONENT$r = ['.ripple'];

  const CLASSNAME_UNBOUND$r = 'ripple-surface-unbound';
  const GRADIENT$r =
    'rgba({{color}}, 0.2) 0, rgba({{color}}, 0.3) 40%, rgba({{color}}, 0.4) 50%, rgba({{color}}, 0.5) 60%, rgba({{color}}, 0) 70%';
  const DEFAULT_RIPPLE_COLOR$r = [0, 0, 0];
  ...
  const TRANSITION_BREAK_OPACITY$r = 0.5;
  ...
  class Ripple {
    constructor(element, options) {
      ...
      if (this._element) {
        Data.set(element, DATA_KEY$r, this);
    ...
    dispose() {
      Data.remove(this._element, DATA_KEY$r);
    ...
    _autoInit(event) {
      SELECTOR_COMPONENT$r.forEach((selector) => {
    ...
    _createRipple(event) {
      ...
      const opacity = {
        delay: duration * TRANSITION_BREAK_OPACITY$r,
        duration: duration - duration * TRANSITION_BREAK_OPACITY$r,
      };
      ...
      const rippleHTML = createElement('div');
    ...
    _getConfig(config = {}) {
      ...
      typeCheckConfig(NAME$r, config, DefaultType$r);
      return config;
    }
    ...
    _toggleUnbound(target) {
      if (this._options.rippleUnbound === true) {
        Manipulator.addClass(target, CLASSNAME_UNBOUND$r);
      } else {
        target.classList.remove(CLASSNAME_UNBOUND$r);
      }
    }
    ...
    _addColor(target, parent) {
      ...
      if (IS_BOOTSTRAP_COLOR) {
        ...
        const gradientImage = GRADIENT$r.split('{{color}}').join(`${rgbValue}`);
    ...
    _colorToRGB(color) {
      ...
      function namedColorsToRgba(color) {
        const tempElem = document.body.appendChild(createElement('fictum'));
    ...
    static jQueryInterface(options) {
      return this.each(function () {
        const data = Data.get(this, DATA_KEY$r);
        if (!data) {
    ...
    static getInstance(element) {
      return Data.get(element, DATA_KEY$r);
    }
  ...
  onDOMContentLoaded(() => {
    ...
    if ($) {
      const JQUERY_NO_CONFLICT = $.fn[NAME$r];
      $.fn[NAME$r] = Ripple.jQueryInterface;
      $.fn[NAME$r].Constructor = Ripple;
      $.fn[NAME$r].noConflict = () => {
        $.fn[NAME$r] = JQUERY_NO_CONFLICT;
