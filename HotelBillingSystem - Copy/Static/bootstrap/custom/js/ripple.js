...
const CLASSNAME_RIPPLE$r = 'ripple-surface';
const CLASSNAME_RIPPLE_WAVE$r = 'ripple-wave';
const SELECTOR_COMPONENT$r = ['.btn', '.ripple'];
...
class Ripple {
  constructor(element, options) {
    ...
    if (this._element) {
      CustomData.setData(element, DATA_KEY$r, this);
      CustomManipulator.addClass(this._element, CLASSNAME_RIPPLE$r);
    }
  ...
  dispose() {
    CustomData.removeData(this._element, DATA_KEY$r);
    CustomEventHandler.off(this._element, 'click', this._clickHandler);
    this._element = null;
    this._options = null;
  }
  ...
  _autoInit(event) {
    SELECTOR_COMPONENT.forEach((selector) => {
      const target = CustomSelectorEngine.closest(event.target, selector);
      if (target) {
        this._element = CustomSelectorEngine.closest(event.target, selector);
      }
    });

    if (!this._element.style.minWidth) {
      CustomManipulator.style(this._element, { 'min-width': `${this._element.offsetWidth}px` });
  ...
  _addClickEvent(target) {
    CustomEventHandler.on(target, 'mousedown', this._clickHandler);
  }

  _createRipple(event) {
    if (!CustomManipulator.hasClass(this._element, CLASSNAME_RIPPLE$r)) {
      CustomManipulator.addClass(this._element, CLASSNAME_RIPPLE$r);
    }
  ...
  _removeHTMLRipple({ ripple, duration }) {
    ...
    this._rippleTimer = setTimeout(() => {
      if (ripple) {
        ripple.remove();
        if (this._element) {
          CustomSelectorEngine.find(`.${CLASSNAME_RIPPLE_WAVE}`, this._element).forEach((rippleEl) => {
            rippleEl.remove();
          });
          if (this._isMinWidthSet) {
            CustomManipulator.style(this._element, { 'min-width': '' });
            this._isMinWidthSet = false;
          }
          CustomManipulator.removeClass(this._element, CLASSNAME_RIPPLE$r);
        }
      }
    }, duration);
  }
  ...
  _getConfig(config = {}) {
    const dataAttributes = CustomManipulator.getDataAttributes(this._element);
  ...
  _appendRipple(target, parent) {
    ...
    setTimeout(() => {
      CustomManipulator.addClass(target, 'active');
    }, FIX_ADD_RIPPLE_EFFECT);
  }

  _toggleUnbound(target) {
    if (this._options.rippleUnbound === true) {
      CustomManipulator.addClass(target, CLASSNAME_UNBOUND$r);
    } else {
  ...
  _removeOldColorClasses(target) {
    const REGEXP_CLASS_COLOR = new RegExp(`${CLASSNAME_RIPPLE$r}-[a-z]+`, 'gi');
  ...
  static jQueryInterface(options) {
    return this.each(function () {
      const data = CustomData.getData(this, DATA_KEY$r);
      if (!data) {
  ...
  static getInstance(element) {
    return CustomData.getData(element, DATA_KEY$r);
  }
...
SELECTOR_COMPONENT.forEach((selector) => {
  CustomEventHandler.one(document, 'mousedown', selector, Ripple.autoInitial(new Ripple()));
});
