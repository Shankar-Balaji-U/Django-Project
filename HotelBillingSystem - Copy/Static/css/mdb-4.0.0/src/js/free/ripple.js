...
const NAME$10 = 'ripple';
const DATA_KEY$10 = 'mdb.ripple';
const CLASSNAME_RIPPLE = 'ripple-surface';
const CLASSNAME_RIPPLE_WAVE = 'ripple-wave';
const SELECTOR_COMPONENT$10 = ['.btn', '.ripple'];

const CLASSNAME_UNBOUND$10 = 'ripple-surface-unbound';
const GRADIENT =
...
class Ripple {
  ...
  _getConfig(config = {}) {
    ...
    typeCheckConfig(NAME$10, config, DefaultType);
    return config;
  }
  ...
  _toggleUnbound(target) {
    if (this._options.rippleUnbound === true) {
      Manipulator.addClass(target, CLASSNAME_UNBOUND$10);
    } else {
      target.classList.remove(CLASSNAME_UNBOUND$10);
    }
  }
...
onDOMContentLoaded(() => {
  ...
  if ($) {
    const JQUERY_NO_CONFLICT = $.fn[NAME$10];
    $.fn[NAME$10] = Ripple.jQueryInterface;
    $.fn[NAME$10].Constructor = Ripple;
    $.fn[NAME$10].noConflict = () => {
      $.fn[NAME$10] = JQUERY_NO_CONFLICT;
